...
describe('AuthApp session lifecycle', () => {
  ...
  it('keeps configured accounts in the header while rendering only open sessions', async () => {
    ...
    expect(root.innerHTML).not.toContain('data-toggle-screens-only')
    expect(root.innerHTML).toContain('data-toggle-ultra')
  ...
  it('enters Ultra without closing games and restores the previous workspace', async () => {
    installBrowser('https://app.example.com/')
    const root = createRoot()
    const auth = createAuthServiceDouble()
    auth.getSession.mockResolvedValue(session)
    let nextId = 0
    const accounts = new ConfiguredAccountService({ storage: null, createId: () => `ultra-game-${++nextId}` })
    const account = accounts.add(user.id, { displayName: 'Conta A', gameSlug: 'huntera' })
    const second = accounts.add(user.id, { displayName: 'Conta B', gameSlug: 'huntera' })
    const permissions = new PermissionService()
    const chatApi = { deleteDirectChat: vi.fn(), getChatChannels: vi.fn().mockResolvedValue({ channels: [] }),
      getChatMessages: vi.fn(), getChatStatus: vi.fn().mockResolvedValue({ status: { banned: false, muted_until: null, reason: null } }),
      reportChatMessage: vi.fn(), sendChatMessage: vi.fn(), startDirectChat: vi.fn() }
    const chat = new ChatService(chatApi, null, null)
    const app = new AuthApp(root, auth.service, { accountService: accounts, chatService: chat, permissionService: permissions })
    await app.start()
    await permissions.openSession(account.id, () => undefined)
    await permissions.openSession(second.id, () => undefined)
    const state = app as unknown as { setUltraMode(enabled: boolean): void; toggleUltraLayout(): void; cycleUltraAccount(direction: 'previous' | 'next'): void; switchToAccountByDigit(digit: string): boolean; screensOnly: boolean; workspaceMode: string; focusedAccountId: string | null; ultraMode: boolean; render(): void }
    state.render()
    expect(root.innerHTML).toContain('data-toggle-ultra')
    state.setUltraMode(true)
    expect(state.ultraMode).toBe(true)
    expect(state.screensOnly).toBe(true)
    expect(state.workspaceMode).toBe('account')
    expect(root.innerHTML).toContain('Voltar ao AltGrid')
    expect(root.innerHTML).toContain('data-ultra-layout')
    expect(root.innerHTML).not.toContain('data-open-chat')
    expect(root.innerHTML).not.toContain('data-utility-cpu')
    expect(permissions.isSessionActive(account.id)).toBe(true)
    expect(permissions.isSessionActive(second.id)).toBe(true)
    expect(chat.getState()).toMatchObject({ open: false, messages: [], channels: [] })
    state.cycleUltraAccount('next')
    expect(state.focusedAccountId).toBe(second.id)
    state.cycleUltraAccount('previous')
    expect(state.focusedAccountId).toBe(account.id)
    expect(state.switchToAccountByDigit('2')).toBe(true)
    expect(state.focusedAccountId).toBe(second.id)
    expect(state.workspaceMode).toBe('account')
    expect(permissions.getActiveSessionCount()).toBe(2)
    state.setUltraMode(false)
    expect(state.ultraMode).toBe(false)
    expect(state.screensOnly).toBe(false)
    expect(state.workspaceMode).toBe('account')
    expect(root.innerHTML).toContain('data-toggle-ultra')
    expect(root.innerHTML).toContain('data-open-chat')
    expect(permissions.isSessionActive(account.id)).toBe(true)
    expect(permissions.isSessionActive(second.id)).toBe(true)
    state.workspaceMode = 'grid'
    state.setUltraMode(true)
    expect(state.workspaceMode).toBe('account')
    state.toggleUltraLayout()
    expect(state.workspaceMode).toBe('grid')
    state.toggleUltraLayout()
    expect(state.workspaceMode).toBe('account')
    state.setUltraMode(false)
    expect(state.workspaceMode).toBe('grid')
  ...
  it('opens three Huntera sessions on FREE, blocks the fourth, shows plan details, and closes active sessions on sign-out', async () => {
    ...
    expect(root.innerHTML).toContain('Proxy exclusivo, salvo e ativado por conta no computador')
    expect(root.innerHTML).toContain('Extensão isolada em até 3 contas no computador')
    expect(root.innerHTML).toContain('Extensão isolada em até 9 contas no computador')
    expect(root.innerHTML).toContain('Extensões ilimitadas, isoladas por conta no computador')
    ...
    // This case verifies an explicit logout, not transient auth recovery.
    auth.emit('SIGNED_OUT', null)
    await Promise.resolve()
    expect(launcher.close).not.toHaveBeenCalled()
    expect(permissions.getActiveSessionCount()).toBe(3)
    expect((app as unknown as { authRecoveryTimer: unknown }).authRecoveryTimer).not.toBeNull()
    auth.emit('TOKEN_REFRESHED', session)
    await Promise.resolve()
    expect((app as unknown as { authRecoveryTimer: unknown }).authRecoveryTimer).toBeNull()
    ;(app as unknown as { intentionalSignOut: boolean }).intentionalSignOut = true
    auth.emit('SIGNED_OUT', null)
