...
begin
  ...
  -- PostgREST maps a JSON request value of null to SQL NULL for RPC
  -- parameters. Normalize it back to JSON null for the one nullable setting.
  if normalized_key = 'founder_max_sales' and p_value is null then
    p_value := 'null'::jsonb;
  end if;
